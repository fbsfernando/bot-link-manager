...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Stop session request:', sessionName)
    ...
    const stopUrl = `${wahaBaseUrl}/api/sessions/${sessionName}/stop`
    console.log('Stopping session via WAHA API:', stopUrl)

    const wahaResponse = await fetch(stopUrl, {
      method: 'POST',
      headers: wahaHeaders,
    })
    ...
    if (!wahaResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ 
          error: `Failed to stop session: ${wahaResponse.status} ${wahaResponse.statusText}`,
          details: errorText
        }),
        { 
